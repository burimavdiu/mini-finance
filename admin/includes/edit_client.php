...
if(isset($_GET['client_id'])){
	...
	$client_res=findClientById($client_id);
...
if(isset($_POST['updateClient'])){
	$mesazhi=updateClient($client_id,
...
<div class="card-body">
	<form method="post" role="form">	
		...
		<div class="form-group col-md-6">
			...
			<div class="form-group col-md-6">
				...
				<select name="clientType" class="form-control" id="clientType" type="text" aria-describedby="clientTypeHelp"  required />
					<option value="Furnitor" <?php if (!empty($client_type) && $client_type == 'Furnitor') echo 'selected = "selected"';?>>Furnitor</option>
					<option value="Të gjitha" <?php if (!empty($client_type) && $client_type == 'Të gjitha') echo 'selected = "selected"';?>>Të gjitha</option>
				</select>
			</div>
		</div>
		<div class="form-row">	
		...
		<input name="updateClient" type="submit" class="btn btn-primary btn-block" value="Modifiko">
	</form>
</div>
</div>